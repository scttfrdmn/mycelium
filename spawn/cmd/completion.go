...
// completeDNSName provides completion for DNS names
// Returns DNS names (short form) from running spawn instances
func completeDNSName(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	...
	// List spawn-managed instances with DNS names
	input := &ec2.DescribeInstancesInput{
		Filters: []ec2types.Filter{
			{
				Name:   stringPtr("tag:spawn:managed"),
	...
	for _, reservation := range result.Reservations {
		for _, instance := range reservation.Instances {
			// Get DNS name from tags (short form)
			...
			// Filter by prefix
			if toComplete != "" && !strings.HasPrefix(dnsName, toComplete) {
				continue
			}

			// Get instance type and ID for description
			instanceType := string(instance.InstanceType)
			instanceID := stringValue(instance.InstanceId)
			description := fmt.Sprintf("%s\t%s (%s)", dnsName, instanceType, instanceID)
			suggestions = append(suggestions, description)
		}
	}
